...
#
# Writes: dist/Fade-to-Black-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Fade-to-Black-Recomp \
  --exe-name Fade_to_Black_Recompiled \
  --display-name "Fade to Black Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Fade to Black disc" \
